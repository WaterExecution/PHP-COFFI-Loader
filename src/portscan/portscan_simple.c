...
/* bof_loader's BeaconPrintf variadic stub can 0xC0000005 after returning to some frames;
 * BeaconOutput(type, buf, len) matches the loader and avoids that path. */
#define BOF_OUT_LINE 2048
#define BOF_MSG(cbtype, fmt, ...)                                                               \
    do {                                                                                        \
        char _bof_l[BOF_OUT_LINE];                                                              \
        int _bof_n = MSVCRT$sprintf(_bof_l, (fmt), ##__VA_ARGS__);                              \
        if (_bof_n > 0 && _bof_n < BOF_OUT_LINE)                                                \
            BeaconOutput((cbtype), _bof_l, _bof_n);                                            \
    } while (0)
...
int parse_cidr_simple(const char* cidr, char*** ip_list, int* count)
{
    ...
    if (mask != 24) {
        BOF_MSG(CALLBACK_ERROR, "Only /24 CIDR notation supported in this version\n");
        return 0;
    }
    ...
    while (token)
    {
        ...
        if (MSVCRT$strchr(token, '/')) {
            if (!parse_cidr_simple(token, &tmp_list, &tmp_count)) {
                BOF_MSG(CALLBACK_ERROR, "Invalid CIDR: %s\n", token);
            ...
            *dash = '\0';
            ...
            if (start_a == INADDR_NONE || end_a == INADDR_NONE) {
                BOF_MSG(CALLBACK_ERROR, "Invalid IP range: %s-%s\n", start_ip, end_ip);
                token = MSVCRT$strtok(NULL, ",");
                continue;
            }
...
void simple_port_scan(const char* target, int scan_level, const char* custom_ports)
{
    ...
    BOF_MSG(CALLBACK_OUTPUT, "[*] Starting port scan for: %s\n", target);

    WSADATA wsa;
    MSVCRT$memset(&wsa, 0, sizeof(wsa));
    if (WS2_32$WSAStartup(MAKEWORD(2,2), &wsa) != 0) {
        BOF_MSG(CALLBACK_ERROR, "WSAStartup failed\n");
        return;
    }
    need_cleanup_wsa = 1;

    if (!parse_target_list_extended(target, &ip_list, &ip_count)) {
        BOF_MSG(CALLBACK_ERROR, "Failed to parse targets: %s\n", target);
        goto cleanup;
    }
    BOF_MSG(CALLBACK_OUTPUT, "[*] Scanning %d hosts\n", ip_count);

    if (scan_level == 0 && custom_ports && custom_ports[0]) {
        ports = parse_custom_ports(custom_ports, &port_count);
        if (ports) {
            BOF_MSG(CALLBACK_OUTPUT, "[*] Using custom ports: %s (%d ports)\n", custom_ports, port_count);
            free_ports = 1;
        } else {
            BOF_MSG(CALLBACK_ERROR, "Invalid custom port specification: %s\n", custom_ports);
            BOF_MSG(CALLBACK_OUTPUT, "[*] Using default port list instead\n");
            scan_level = 2;
        }
    }
    ...
    if (scan_level > 0) {
        switch (scan_level) {
            case 1:
                ...
                BOF_MSG(CALLBACK_OUTPUT, "[*] Level 1 scan: Web & Database ports (%d ports)\n", port_count);
                break;
            case 2:
                ports = g_level2_ports;
                for (port_count = 0; g_level2_ports[port_count] != 0; port_count++);
                BOF_MSG(CALLBACK_OUTPUT, "[*] Level 2 scan: Web, Database & OS-specific ports (%d ports)\n", port_count);
                break;
            case 3:
                ports = g_level3_ports;
                for (port_count = 0; g_level3_ports[port_count] != 0; port_count++);
                BOF_MSG(CALLBACK_OUTPUT, "[*] Level 3 scan: Comprehensive ports (%d ports)\n", port_count);
                break;
            default:
                ports = g_level2_ports;
                for (port_count = 0; g_level2_ports[port_count] != 0; port_count++);
                BOF_MSG(CALLBACK_OUTPUT, "[*] Default scan: Web, Database & OS-specific ports (%d ports)\n", port_count);
        }
    }

    BOF_MSG(CALLBACK_OUTPUT, "[*] Scanning %d ports\n", port_count);

    int total_open_ports = 0;
    for (int i = 0; i < ip_count; i++) {
        BOF_MSG(CALLBACK_OUTPUT, "[*] Scanning %s...\n", ip_list[i]);
        ...
        for (int batch_start = 0; batch_start < port_count; batch_start += SCAN_BATCH_SIZE) {
            ...
            for (int j = batch_start; j < batch_end; j++) {
                ...
                if (state > 0) {
                    BOF_MSG(CALLBACK_OUTPUT, "  [+] Port %d (%s) - OPEN\n",
        ...
        if (host_open_ports > 0) {
            BOF_MSG(CALLBACK_OUTPUT, "[+] Host %s: %d open ports found\n",
                       ip_list[i], host_open_ports);
        }

        if (ip_count > 10 && (i + 1) % 10 == 0) {
            BOF_MSG(CALLBACK_OUTPUT, "[*] Progress: %d/%d hosts scanned\n", i + 1, ip_count);
        }
    }

    BOF_MSG(CALLBACK_OUTPUT, "[+] Scan completed. Found %d open ports total.\n", total_open_ports);
...
__attribute__((noinline)) static void go_run_scan(char* target, int scan_level, char* custom_ports)
{
...
int go(char* args, int len)
{
    ...
    if (target_ptr && target_size > 0) {
        ...
        BOF_MSG(CALLBACK_ERROR, "Parameters are not supported\n");
        return 0;
    }
    ...
    go_run_scan(target, scan_level, custom_ports);
